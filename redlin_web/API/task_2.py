...
from .models import Document, Summary, Flashcard, MCQ, Cloze, Feynman
...
# ---------------------------------------------------------------------------
# Feynman (concept explanation prompts) generation
# ---------------------------------------------------------------------------

def _ai_feynman_prompt(source_text: str, *, lang_label: str, soft_cap: int | None) -> str:
    """Build prompt for generating Feynman prompts + key points.

    soft_cap: advisory maximum of items (None = unlimited). The model is instructed
    to stop when additional prompts would be redundant.
    """
    snippet = source_text[:16000]
    cap_text = (
        f"Aim for at most ~{soft_cap} distinct prompts if the text supports that many; "
        "stop earlier when coverage is complete."
        if soft_cap and soft_cap > 0 else
        "Generate as many distinct prompts as needed for full coverage; stop when concepts become redundant."
    )
    language_line = "Idioma de salida: Español" if lang_label == "Spanish" else "Output language: English"
    # We use a strict JSON requirement to ease downstream parsing
    return (
...
def _clean_ai_feynman_json(raw: str) -> dict | None:
    """Extract JSON for Feynman generation; reuse generic logic but look for 'items'."""
    raw = raw.strip()
    try:
        data = json.loads(raw)
    ...
    if raw.startswith("```"):
        segments = raw.split("```")
        raw = "\n".join(s for s in segments if 'items' in s or '{' in s)
        raw = raw.strip()
    obj = _extract_json_block(raw, key_hint='"items"')
    if obj:
        try:
            data = json.loads(obj)
            if isinstance(data, dict) and 'items' in data:
...
def generate_ai_feynman(document: Document, source_text: str, lang_label: str, *, soft_cap: int | None, regenerate: bool) -> list[Feynman]:
    """Generate Feynman prompts + key points.

    soft_cap: advisory maximum number of items (None or 0 -> unlimited but internally safety capped)
    regenerate: if False and items already exist -> skip
    """
    ...
    # Safety hard cap to prevent runaway token usage even if unlimited requested
    internal_max = None
    if soft_cap and soft_cap > 0:
        internal_max = soft_cap
    else:
        internal_max = 80  # internal safety upper bound
    ...
    for label, prompt in attempts:
        try:
            ...
            print(f"[Feynman] {label} generation failed: {e}")
            continue
        raw_text = getattr(resp, 'text', '') or ''
    ...
    for idx, item in enumerate(items_iter, start=1):
        ...
        # Enforce preferred length cap (soft) by trimming mid-word if > 140
        ...
        for kp in raw_points:
            if isinstance(kp, dict):
                ...
                if weight > 1.5:
                    weight = 1.5
                structured_points.append({"id": len(structured_points)+1, "point": point, "weight": round(weight, 2)})
            elif isinstance(kp, str):
                point = kp.strip()
                if point:
                    structured_points.append({"id": len(structured_points)+1, "point": point, "weight": 1.0})
        if len(structured_points) < 2:  # need at least minimal guidance
            continue
        try:
            ...
            print(f"[Feynman] Persist error: {e} | Raw item: {snippet}")
...
"""
        ...
        # ---- Generate Feynman Prompts ----
        try:
            print("Generating Feynman prompts...")
            # Compose source: leverage summary (concept condensation) + raw slice for coverage
            combined_source = (summary_content or "") + "\n\n" + text[:12000]
            # Env configuration
            cap_env = os.getenv("AI_FEYNMAN_MAX", "").strip()
            soft_cap = None
            if cap_env.isdigit() and int(cap_env) > 0:
                soft_cap = int(cap_env)
            regenerate = os.getenv("AI_FEYNMAN_REGENERATE", "0").lower() in {"1","true","yes","on"}
            created_feynman = generate_ai_feynman(document, combined_source, lang_label, soft_cap=soft_cap, regenerate=regenerate)
            print(f"Feynman prompts total now: {document.feynmans.count()} (new {len(created_feynman)}).")
        except Exception as e:
            print(f"[Error] Feynman generation failed: {e}")
