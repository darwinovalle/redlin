...
from API.services.processing_common import extract_json_block, generate_with_retry

from ..models import ClassSession, ClassSessionFeynman


def _ai_feynman_prompt(source_text: str, *, lang_label: str, soft_cap: int | None) -> str:
    """Build prompt for generating classroom Feynman prompts and key points."""
...
def _clean_ai_feynman_json(raw: str) -> dict | None:
    """Extract JSON payload for classroom feynman generation."""
    raw = raw.strip()
    try:
        data = json.loads(raw)
...
def generate_ai_session_feynman(
    ...
    *,
    soft_cap: int | None,
    regenerate: bool,
) -> list[ClassSessionFeynman]:
    """Generate and persist Feynman prompts for a class session."""
    existing_count = class_session.feynmans.count()
    if existing_count > 0 and not regenerate:
        print(f"[Classroom Feynman] Existing items ({existing_count}) present; skipping generation (regenerate disabled).")
        return []
    ...
    if not payload:
        print("[Classroom Feynman] Invalid JSON structure after attempts")
        return []

    created: list[ClassSessionFeynman] = []
    ...
    for item in items_iter:
        ...
        try:
            f_obj = ClassSessionFeynman.objects.create(
            ...
            print(f"[Classroom Feynman] Persist error: {exc} | Raw item: {snippet}")

    print(f"[Classroom Feynman] Created {len(created)} items (requested soft cap {soft_cap}).")
    return created


def generate_session_feynman(class_session: ClassSession, text: str, summary_content: str, lang_label: str) -> int:
    """Generate classroom Feynman prompts and return the created count."""
    try:
        print("Generating classroom Feynman prompts...")
        ...
        created = generate_ai_session_feynman(
            class_session,
            combined_source,
            lang_label,
            soft_cap=soft_cap,
        ...
        print(f"Classroom Feynman prompts total now: {class_session.feynmans.count()} (new {len(created)}).")
        return len(created)
    except Exception as exc:
        print(f"[Error] Classroom Feynman generation failed: {exc}")
        return 0